...
class RatingRenderer
{
    ...
    public static function stars_html($rating, $size = 16, $extra_class = '')
    {
        ...
        for ($star = 1; $star <= 5; $star++) {
            if ($rating >= $star) {
                $html .= self::icon('star-fill', $size, $extra_class);
                continue;
            }

            if ($rating === ($star - 0.5)) {
                $html .= self::icon('star-half', $size, trim($extra_class . ' wps-rating-star-half'));
                continue;
            }

            $html .= self::icon('star-fill', $size, trim($extra_class . ' wps-opacity-25'));
        }
    ...
    public static function summary_html($rating, $count = null, $args = [])
    {
        ...
        $count_text = isset($args['count_text']) ? (string) $args['count_text'] : __('ulasan', 'vd-store');

        $parts = [];
        $parts[] = sprintf(
            '<span class="%1$s" aria-label="%2$s">%3$s</span>',
            esc_attr('wps-inline-flex wps-items-center wps-gap-1 wps-text-warning'),
        ...
        if ($show_count && $count !== null) {
            $parts[] = sprintf(
                ...
                esc_attr('wps-text-sm wps-text-gray-500'),
                esc_html(sprintf(__('%1$d %2$s', 'vd-store'), (int) $count, $count_text))
            );
        }

        return sprintf(
            '<span class="%1$s">%2$s</span>',
            esc_attr(trim('wps-inline-flex wps-flex-wrap wps-items-center wps-gap-2 ' . $class)),
            implode('', $parts)
        );
    }
    ...
    private static function icon($type, $size, $class_attr)
    {
        ...
        return sprintf(
            ...
            (int) $size,
            esc_attr(trim((string) $class_attr)),
            esc_attr($path)
        );
    }
}
