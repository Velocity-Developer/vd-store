...
### Field tambahan checkout

Gunakan filter `wp_store_checkout_fields` dari plugin kustom atau child theme
untuk menambah field pada checkout VD Store dan Velocity Marketplace yang sudah
mendukung integrasi ini. Tersedia tipe `text`, `textarea`, `email`, `tel`,
`select`, dan `checkbox`, dengan pengaturan label, placeholder, wajib isi, dan urutan serta section.

Field otomatis dikirim, divalidasi di server, dan disimpan pada metadata pesanan
`_store_order_checkout_fields`. Nilainya dapat dilihat di box **Field Tambahan
Checkout** pada editor pesanan WordPress. Filter ini hanya mengatur field tambahan,
belum field bawaan checkout. Tidak ada field tambahan aktif secara default.
